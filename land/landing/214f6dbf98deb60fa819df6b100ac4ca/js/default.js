...
function second_scene(){
	...
	setTimeout("third_secene()",3000);				// 3번씬으로 넘어가는 시간 3000=3초
}
...
function prograss_2(){	
	var pGress2 = setInterval(function() {
        var pVal2 = parseInt($("#progressbar2").data("value"));

        if(pVal2 > 100) {
			clearInterval(pGress2);
            prograss_3();
        } else {
            pVal2++;
...
function prograss_3(){	
	var pGress3 = setInterval(function() {
        var pVal3 = parseInt($("#progressbar3").data("value"));
